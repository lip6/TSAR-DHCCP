...
init {
	...
	chan OBSREQX    = [1] of {msg} ;
	chan OBSACKX    = [1] of {msg} ;
	chan OBSREQY    = [1] of {msg} ;
	chan OBSACKY    = [1] of {msg} ;
	chan OBSRDX     = [1] of {msg} ;
	chan OBSACKRDX  = [1] of {msg} ;
	chan OBSRDY     = [1] of {msg} ;
	chan OBSACKRDY  = [1] of {msg} ;

	atomic {
	//processeur.
	run proc(PL1DTREQ, L1PDTACK, OBSREQX, OBSACKX, OBSREQY, OBSACKY, OBSRDX,OBSACKRDX,OBSRDY,OBSACKRDY) ;
	// Cache L1.
	...
	run obs_rd_after_wr(OBSREQX, OBSACKX, X) ;
	// Observateurs p2 pour Y.
	run obs_rd_after_wr(OBSREQY, OBSACKY, Y) ;
	// Observateurs p3 pour X.
	run obs_rd_after_rd(OBSRDX, OBSACKRDX, X) ;
	// Observateurs p3 pour Y.
	run obs_rd_after_rd(OBSRDY, OBSACKRDY, Y) ;
